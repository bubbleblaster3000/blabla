# 🧪 Kapitel 13: Kolligative Eigenschaften - Level 2

**Schwierigkeitsgrad:** Leicht-Mittel - Anwendung einfacher Konzepte

---

## Frage 1
**Die Formel für die Gefrierpunktserniedrigung lautet ΔTf = Kf × m. Was ist Kf?**

- [ ] A. Die kryoskopische Konstante (spezifisch für das Lösungsmittel)
- [ ] B. Die Molalität
- [ ] C. Die Molmasse
- [ ] D. Die Konzentration

**Richtige Antworten:** A

---

## Frage 2
**Die kryoskopische Konstante von Wasser ist Kf = 1,86 K·kg/mol. Berechne ΔTf für eine 1 m Glucose-Lösung.**

- [ ] A. 1,86°C
- [ ] B. 3,72°C
- [ ] C. 0,93°C
- [ ] D. 5,58°C

**Richtige Antworten:** A

---

## Frage 3
**Die Formel für die Siedepunkterhöhung lautet ΔTb = Kb × m. Die ebullioskopische Konstante von Wasser ist Kb = 0,512 K·kg/mol. Berechne ΔTb für eine 2 m Lösung.**

- [ ] A. 1,024°C
- [ ] B. 0,512°C
- [ ] C. 2,048°C
- [ ] D. 0,256°C

**Richtige Antworten:** A

---

## Frage 4
**Das Raoult'sche Gesetz lautet: P = x × P°. Was ist x?**

- [ ] A. Der Molenbruch des Lösungsmittels
- [ ] B. Die Molalität
- [ ] C. Die Masse des gelösten Stoffes
- [ ] D. Die Temperatur

**Richtige Antworten:** A

---

## Frage 5
**Warum führt ein gelöster nicht-flüchtiger Stoff zur Dampfdruckerniedrigung?**

- [ ] A. Der gelöste Stoff blockiert Lösungsmittelmoleküle an der Oberfläche
- [ ] B. Der Molenbruch des Lösungsmittels sinkt
- [ ] C. Weniger Lösungsmittelmoleküle können verdampfen
- [ ] D. Der gelöste Stoff erhöht den Dampfdruck

**Richtige Antworten:** A, B, C

---

## Frage 6
**Die van't Hoff-Gleichung für den osmotischen Druck lautet: π = MRT. Was bedeuten die Symbole?**

- [ ] A. π = osmotischer Druck, M = Molarität, R = Gaskonstante, T = Temperatur
- [ ] B. π = Dampfdruck, M = Masse, R = Radius, T = Zeit
- [ ] C. π = Pi, M = Molmasse, R = Reaktionsgeschwindigkeit, T = Temperatur
- [ ] D. π = Partialdruck, M = Molalität, R = Rydberg-Konstante, T = Tripelpunkt

**Richtige Antworten:** A

---

## Frage 7
**Eine 0,1 M Glucose-Lösung hat bei 25°C einen osmotischen Druck von etwa:**

- [ ] A. 2,45 atm
- [ ] B. 0,245 atm
- [ ] C. 24,5 atm
- [ ] D. 0,0245 atm

**Richtige Antworten:** A

---

## Frage 8
**Welche Lösung hat den niedrigsten Gefrierpunkt?**

- [ ] A. 1 m NaCl
- [ ] B. 1 m Glucose
- [ ] C. 0,5 m CaCl₂
- [ ] D. Alle haben den gleichen Gefrierpunkt

**Richtige Antworten:** A

---

## Frage 9
**Was ist der van't Hoff-Faktor i?**

- [ ] A. Die Anzahl der Teilchen, in die eine Formeleinheit dissoziiert
- [ ] B. Die Molmasse des gelösten Stoffes
- [ ] C. Die Dichte der Lösung
- [ ] D. Die Temperaturänderung

**Richtige Antworten:** A

---

## Frage 10
**Für welchen Stoff ist i = 3?**

- [ ] A. CaCl₂ (Ca²⁺ + 2 Cl⁻)
- [ ] B. NaCl (Na⁺ + Cl⁻)
- [ ] C. Glucose (dissoziiert nicht)
- [ ] D. AlCl₃ (Al³⁺ + 3 Cl⁻)

**Richtige Antworten:** A

---
