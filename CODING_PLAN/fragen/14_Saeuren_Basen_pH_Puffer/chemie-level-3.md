# 🧪 Kapitel 14: Säuren & Basen (pH, Puffer, Titrationen) - Level 3

**Schwierigkeitsgrad:** Mittel - Verknüpfung mehrerer Konzepte

---

## Frage 1
**Berechne den pH-Wert einer 0,1 M Essigsäure-Lösung. (pKs = 4,76)**

- [ ] A. ca. 2,88
- [ ] B. ca. 4,76
- [ ] C. ca. 1,00
- [ ] D. ca. 7,00

**Richtige Antworten:** A

---

## Frage 2
**Berechne den pH-Wert einer 0,1 M Natriumacetat-Lösung. (pKs der Essigsäure = 4,76)**

- [ ] A. ca. 8,88
- [ ] B. ca. 4,76
- [ ] C. ca. 7,00
- [ ] D. ca. 5,12

**Richtige Antworten:** A

---

## Frage 3
**Ein Puffer enthält 0,2 M Essigsäure und 0,1 M Natriumacetat. Berechne den pH. (pKs = 4,76)**

- [ ] A. 4,46
- [ ] B. 4,76
- [ ] C. 5,06
- [ ] D. 5,76

**Richtige Antworten:** A

---

## Frage 4
**Zu dem Puffer aus Frage 3 werden 0,01 mol HCl pro Liter gegeben. Wie ändert sich der pH?**

- [ ] A. Der pH sinkt geringfügig (auf ca. 4,36)
- [ ] B. Der pH sinkt stark auf 1
- [ ] C. Der pH bleibt genau gleich
- [ ] D. Der pH steigt

**Richtige Antworten:** A

---

## Frage 5
**Bei der Titration einer schwachen Säure mit einer starken Base liegt der Äquivalenzpunkt:**

- [ ] A. Im basischen Bereich (pH > 7)
- [ ] B. Bei pH = 7
- [ ] C. Im sauren Bereich (pH < 7)
- [ ] D. Kann nicht bestimmt werden

**Richtige Antworten:** A

---

## Frage 6
**Was ist der Halbtitrationsunkt?**

- [ ] A. Der Punkt, an dem die Hälfte der Säure neutralisiert ist und pH = pKs
- [ ] B. Der Äquivalenzpunkt
- [ ] C. Der Startpunkt der Titration
- [ ] D. Der Punkt bei pH = 7

**Richtige Antworten:** A

---

## Frage 7
**Welcher Indikator ist für die Titration einer schwachen Säure mit starker Base geeignet?**

- [ ] A. Phenolphthalein (Umschlagbereich 8,2-10)
- [ ] B. Methylorange (Umschlagbereich 3,1-4,4)
- [ ] C. Beide sind gleich gut
- [ ] D. Keiner ist geeignet

**Richtige Antworten:** A

---

## Frage 8
**Die Pufferkapazität ist am größten, wenn:**

- [ ] A. [HA] = [A⁻] (pH = pKs)
- [ ] B. [HA] >> [A⁻]
- [ ] C. [A⁻] >> [HA]
- [ ] D. Die Konzentration sehr niedrig ist

**Richtige Antworten:** A

---

## Frage 9
**Das Ionenprodukt des Wassers Kw = [H⁺][OH⁻] = 10⁻¹⁴ gilt bei 25°C. Wie ändert sich Kw mit der Temperatur?**

- [ ] A. Kw steigt mit der Temperatur (Dissoziation ist endotherm)
- [ ] B. Kw sinkt mit der Temperatur
- [ ] C. Kw bleibt konstant
- [ ] D. Kw wird negativ

**Richtige Antworten:** A

---

## Frage 10
**Eine mehrprotonige Säure wie H₃PO₄ hat mehrere pKs-Werte. Warum?**

- [ ] A. Jedes Proton hat eine andere Dissoziationskonstante
- [ ] B. Die Abgabe jedes weiteren Protons wird schwieriger
- [ ] C. pKs1 < pKs2 < pKs3
- [ ] D. Jedes Proton hat die gleiche Dissoziationskonstante

**Richtige Antworten:** A, B, C

---
