# 🧪 Kapitel 15: Elektrochemie, Redox & Elektrolyse - Level 4

**Schwierigkeitsgrad:** Schwer - Berechnungen & komplexe Zusammenhänge

---

## Frage 1
**Berechne die Gleichgewichtskonstante K für die Reaktion Zn + Cu²⁺ ⇌ Zn²⁺ + Cu bei 25°C. (E° = 1,10 V)**

- [ ] A. ca. 1,5 × 10³⁷
- [ ] B. ca. 1,5 × 10¹⁸
- [ ] C. ca. 1,5 × 10⁻³⁷
- [ ] D. ca. 1,10

**Richtige Antworten:** A

---

## Frage 2
**Bei der Elektrolyse einer NaCl-Lösung an der Anode: Warum entsteht Cl₂ statt O₂, obwohl E°(O₂/H₂O) < E°(Cl₂/Cl⁻)?**

- [ ] A. Überspannung bei der O₂-Entwicklung ist hoch
- [ ] B. Die Kinetik der Cl₂-Bildung ist schneller
- [ ] C. Bei hoher Cl⁻-Konzentration verschiebt sich das Potential
- [ ] D. Bei hoher Cl⁻-Konzentration entsteht bevorzugt O₂

**Richtige Antworten:** A, B, C

---

## Frage 3
**Eine Brennstoffzelle nutzt die Reaktion H₂ + ½ O₂ → H₂O. Berechne die theoretische EMK. (E°(O₂/H₂O) = +1,23 V, E°(H⁺/H₂) = 0 V)**

- [ ] A. 1,23 V
- [ ] B. 2,46 V
- [ ] C. 0,62 V
- [ ] D. 0 V

**Richtige Antworten:** A

---

## Frage 4
**Der Wirkungsgrad einer Wasserstoff-Brennstoffzelle ist theoretisch höher als der einer Wärmekraftmaschine. Warum?**

- [ ] A. Direkte Umwandlung chemischer in elektrische Energie
- [ ] B. Keine Carnot-Limitierung
- [ ] C. ΔG/ΔH kann über 80% betragen
- [ ] D. Der Wirkungsgrad von Brennstoffzellen ist durch Carnot limitiert

**Richtige Antworten:** A, B, C

---

## Frage 5
**Berechne den pH einer Wasserstoffelektrode mit P(H₂) = 1 atm und E = -0,118 V gegen SHE.**

- [ ] A. pH = 2
- [ ] B. pH = 4
- [ ] C. pH = 0
- [ ] D. pH = 7

**Richtige Antworten:** A

---

## Frage 6
**Kathodischer Korrosionsschutz nutzt eine Opferanode. Welches Metall ist geeignet, um Eisen zu schützen?**

- [ ] A. Zink (E° = -0,76 V)
- [ ] B. Kupfer (E° = +0,34 V)
- [ ] C. Silber (E° = +0,80 V)
- [ ] D. Gold (E° = +1,50 V)

**Richtige Antworten:** A

---

## Frage 7
**Die Chlor-Alkali-Elektrolyse produziert Cl₂, H₂ und NaOH. Welche Gesamtgleichung beschreibt den Prozess?**

- [ ] A. 2 NaCl + 2 H₂O → Cl₂ + H₂ + 2 NaOH
- [ ] B. NaCl + H₂O → NaOH + HCl (keine Elektrolyse)
- [ ] C. 2 NaCl → 2 Na + Cl₂ (Schmelzflusselektrolyse)
- [ ] D. NaCl + H₂O → NaClO + H₂ (Hypochlorit)

**Richtige Antworten:** A

---

## Frage 8
**Ein Bleiakkumulator hat die Reaktion: Pb + PbO₂ + 2 H₂SO₄ ⇌ 2 PbSO₄ + 2 H₂O. Die EMK ist etwa:**

- [ ] A. 2,0 V pro Zelle
- [ ] B. 1,5 V pro Zelle
- [ ] C. 1,2 V pro Zelle
- [ ] D. 3,0 V pro Zelle

**Richtige Antworten:** A

---

## Frage 9
**Bei der Elektrolyse von geschmolzenem NaCl: Was passiert an den Elektroden?**

- [ ] A. Kathode: Na⁺ + e⁻ → Na; Anode: 2 Cl⁻ → Cl₂ + 2e⁻
- [ ] B. Kathode: Cl₂-Entwicklung; Anode: Na-Abscheidung
- [ ] C. An beiden Elektroden entsteht NaCl
- [ ] D. wenige Reaktion

**Richtige Antworten:** A

---

## Frage 10
**Die Standardzellspannung einer Lithium-Ionen-Batterie (LiCoO₂/Graphit) beträgt etwa 3,7 V. Warum ist sie höher als bei einem Bleiakkumulator?**

- [ ] A. Lithium hat ein sehr negatives Standardpotential (-3,04 V)
- [ ] B. Der große Potentialunterschied zwischen Anode und Kathode
- [ ] C. Organische Elektrolyte ermöglichen höhere Spannungen
- [ ] D. Die Lithium-Ionen-Batterie hat niedrigere Spannung als der Bleiakku

**Richtige Antworten:** A, B, C

---
