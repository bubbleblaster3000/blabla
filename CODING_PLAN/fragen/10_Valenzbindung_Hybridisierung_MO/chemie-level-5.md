# 🧪 Kapitel 10: Valenzbindung, Hybridisierung & MO-Theorie - Level 5

**Schwierigkeitsgrad:** Experte - Transfer & anspruchsvolle Problemlösung

---

## Frage 1
**Das Molekül C₂ existiert im Gasphase. Bestimme seine Bindungsordnung und magnetische Eigenschaft nach MO-Theorie.**

- [ ] A. Bindungsordnung 2, diamagnetisch
- [ ] B. Bindungsordnung 2, paramagnetisch
- [ ] C. Bindungsordnung 3, diamagnetisch
- [ ] D. Bindungsordnung 1, paramagnetisch

**Richtige Antworten:** A

---

## Frage 2
**Das Cyanid-Ion (CN⁻) ist isoelektronisch zu welchen Molekülen?**

- [ ] A. N₂ und CO
- [ ] B. O₂ und NO
- [ ] C. F₂ und Cl₂
- [ ] D. H₂ und He₂

**Richtige Antworten:** A

---

## Frage 3
**Im Cyclopentadienyl-Anion (C₅H₅⁻) sind wie viele π-Elektronen delokalisiert und warum ist es aromatisch?**

- [ ] A. 6 π-Elektronen, erfüllt die Hückel-Regel
- [ ] B. 4 π-Elektronen, antiaromatisch
- [ ] C. 5 π-Elektronen, nicht aromatisch
- [ ] D. 8 π-Elektronen, erfüllt die Hückel-Regel

**Richtige Antworten:** A

---

## Frage 4
**Das HOMO-LUMO-Konzept erklärt die Reaktivität von Molekülen. Was trifft zu?**

- [ ] A. HOMO = Highest Occupied Molecular Orbital
- [ ] B. LUMO = Lowest Unoccupied Molecular Orbital
- [ ] C. Eine kleine HOMO-LUMO-Lücke bedeutet höhere Reaktivität
- [ ] D. Die HOMO-LUMO-Lücke ist irrelevant für die Reaktivität

**Richtige Antworten:** A, B, C

---

## Frage 5
**Im Kohlenmonoxid (CO) zeigt das Dipolmoment in Richtung C. Warum?**

- [ ] A. Freies e⁻-Paar am C überwiegt den EN-Unterschied
- [ ] B. Kohlenstoff ist elektronegativer als Sauerstoff im Molekül
- [ ] C. CO hat kein Dipolmoment, es ist unpolar
- [ ] D. Das Dipolmoment zeigt häufig zum elektronegativeren Atom

**Richtige Antworten:** A

---

## Frage 6
**Das Dioxygen-Kation (O₂⁺) wird in Salzen wie O₂⁺[PtF₆]⁻ gefunden. Vergleiche O₂⁺ mit O₂.**

- [ ] A. O₂⁺ hat eine kürzere Bindung als O₂
- [ ] B. O₂⁺ hat eine höhere Bindungsordnung als O₂
- [ ] C. O₂⁺ ist paramagnetisch mit 1 ungepaarten Elektron
- [ ] D. LUMO bezeichnet das niedrigste besetzte Orbital

**Richtige Antworten:** A, B, C

---

## Frage 7
**Erkläre, warum das Butadien-Molekül (H₂C=CH-CH=CH₂) eine partielle Doppelbindungscharakter zwischen C2 und C3 zeigt.**

- [ ] A. Konjugation der π-Elektronen über alle 4 C-Atome
- [ ] B. Delokalisierung führt zu einer Bindungsordnung > 1 zwischen C2-C3
- [ ] C. Das Molekül hat Resonanzstrukturen
- [ ] D. O₂⁺ hat eine niedrigere Bindungsordnung als O₂

**Richtige Antworten:** A, B, C

---

## Frage 8
**Das Fluorid-Ion F⁻ reagiert als Lewis-Base. Welches Orbital stellt das Elektronenpaar bereit?**

- [ ] A. Ein gefülltes 2p-Orbital
- [ ] B. Ein leeres 2s-Orbital
- [ ] C. Ein sp³-Hybridorbital
- [ ] D. Ein antibindendes Orbital

**Richtige Antworten:** A

---

## Frage 9
**Das Molekül Diborane (B₂H₆) enthält 2-Elektronen-3-Zentren-Bindungen. Was bedeutet das?**

- [ ] A. Ein Elektronenpaar wird von drei Atomen geteilt
- [ ] B. Es gibt Brücken-Wasserstoffatome zwischen den Boratomen
- [ ] C. Normale VB-Theorie kann diese Bindung nicht erklären
- [ ] D. Konjugation führt zu lokalisierten π-Elektronen

**Richtige Antworten:** A, B, C

---

## Frage 10
**Das Aufbauprinzip für Molekülorbitale folgt denselben Regeln wie für Atomorbitale. Ordne die MOs für O₂ nach steigender Energie.**

- [ ] A. σ₂s < σ*₂s < σ₂p < π₂p < π*₂p < σ*₂p
- [ ] B. σ₂s < σ*₂s < π₂p < σ₂p < π*₂p < σ*₂p
- [ ] C. π₂p < σ₂s < σ*₂s < σ₂p < π*₂p < σ*₂p
- [ ] D. σ₂s < σ*₂s < σ₂p < π₂p < σ*₂p < π*₂p

**Richtige Antworten:** A

---
