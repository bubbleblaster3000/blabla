# 🧪 Kapitel 6: Säuren & Basen (Grundlagen) - Level 1

**Schwierigkeitsgrad:** Einfach - Definitionen & Grundbegriffe

---

## Frage 1
**Was ist eine Säure nach Arrhenius?**

- [ ] A. Ein Stoff, der OH⁻-Ionen abgibt
- [ ] B. Ein Stoff, der H⁺-Ionen in wässriger Lösung abgibt
- [ ] C. Ein Stoff, der Elektronen aufnimmt
- [ ] D. Ein Stoff, der meist mit Metallen reagiert

**Richtige Antworten:** B

---

## Frage 2
**Was ist eine Base nach Arrhenius?**

- [ ] A. Ein Stoff, der H⁺-Ionen abgibt
- [ ] B. Ein Stoff, der OH⁻-Ionen in wässriger Lösung abgibt
- [ ] C. Ein Stoff, der Elektronen abgibt
- [ ] D. Ein Stoff, der meist mit Nichtmetallen reagiert

**Richtige Antworten:** B

---

## Frage 3
**Was entsteht bei einer Neutralisationsreaktion?**

- [ ] A. meist Wasser
- [ ] B. meist Salz
- [ ] C. Salz und Wasser
- [ ] D. Säure und Base

**Richtige Antworten:** C

---

## Frage 4
**Welche der folgenden ist eine starke Säure?**

- [ ] A. Essigsäure (CH₃COOH)
- [ ] B. Salzsäure (HCl)
- [ ] C. Kohlensäure (H₂CO₃)
- [ ] D. Blausäure (HCN)

**Richtige Antworten:** B

---

## Frage 5
**Welche der folgenden ist eine starke Base?**

- [ ] A. Ammoniak (NH₃)
- [ ] B. Natriumhydroxid (NaOH)
- [ ] C. Aluminiumhydroxid (Al(OH)₃)
- [ ] D. Ammoniumhydroxid (NH₄OH)

**Richtige Antworten:** B

---

## Frage 6
**Was bedeutet es, wenn eine Säure "stark" ist?**

- [ ] A. Sie hat eine hohe Konzentration
- [ ] B. Sie dissoziiert vollständig in Wasser
- [ ] C. Sie ist chemisch gefährlich (ätzend)
- [ ] D. Sie hat einen hohen pH-Wert

**Richtige Antworten:** B

---

## Frage 7
**Was ist das Produkt der Reaktion: HCl + NaOH → ?**

- [ ] A. NaCl + H₂
- [ ] B. NaCl + H₂O
- [ ] C. Na + Cl₂ + H₂O
- [ ] D. NaH + ClOH

**Richtige Antworten:** B

---

## Frage 8
**Welche Ionen sind für den sauren Charakter einer Lösung verantwortlich?**

- [ ] A. OH⁻-Ionen
- [ ] B. H⁺-Ionen (H₃O⁺)
- [ ] C. Na⁺-Ionen
- [ ] D. Cl⁻-Ionen

**Richtige Antworten:** B

---

## Frage 9
**Was ist eine schwache Säure?**

- [ ] A. Eine Säure mit niedriger Konzentration
- [ ] B. Eine Säure, die nur teilweise dissoziiert
- [ ] C. Eine Säure, die nicht reagiert
- [ ] D. Eine verdünnte Säure

**Richtige Antworten:** B

---

## Frage 10
**Welche Aussage über die Neutralisation ist korrekt?**

- [ ] A. Sie ist häufig endotherm
- [ ] B. Säure und Base reagieren zu Salz und Wasser
- [ ] C. Es entstehen häufig Gase
- [ ] D. Der pH-Wert sinkt häufig in der Reaktion

**Richtige Antworten:** B

---
